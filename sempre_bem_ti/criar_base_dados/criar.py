...
data_final = datetime(2023, 12, 30)
...
def rd_produto():
    return random.choice(list(lista_produtos.index))
...
def minimo_maximo(valor):
    return (valor*0.3), (valor*valor*0.7)

# Função para gerar um preço aleatório
def rd_valor(valor):
    return round(random.uniform(valor * 0.3, valor * 1.7), 2)


def rd_quantidade(qtd_maxima):
    return random.randint(1, qtd_maxima)


# Função para gerar um custo de aquisição aleatório
def rd_custo(valor_produto):
    return round(random.uniform(valor_produto * 0.3, valor_produto * 0.85), 2)
...
with open(caminho_base_dados, mode='w', newline='', encoding='utf-8') as file:
    ...
    for i in range(total_registros):
        data = rd_data()
        id_produto = rd_produto()
        produto = lista_produtos.iloc[id_produto,0]
        categoria_produto = lista_produtos.iloc[id_produto,1]
        valor_estimado = lista_produtos.iloc[id_produto,2]
        quantidade = rd_quantidade(5)
        tipo = rd_tipo()
        valor_unitario = rd_valor(valor_estimado)
